...
export class PolarisPanel {
  ...
  private constructor(
    ...
    const savedPrefs =
      this.context.globalState.get<TogglePreferences>(TOGGLE_PREFS_KEY);
    ...
    switch (message.type) {
      ...
      case "queryChanged":
        this.handleQueryChanged(
          message.query,
          message.includeGlobs,
          message.excludeGlobs,
        );
        break;
      case "resultSelected":
  ...
  private rerunSearch(): void {
    if (this.lastQuery.trim()) {
      this.handleQueryChanged(
        this.lastQuery,
        this.lastIncludeGlobs,
        this.lastExcludeGlobs,
      );
    }
  }
  ...
  private async handleResultSelected(
    filePath: string,
    line?: number,
  ): Promise<void> {
    const workspaceRoot = this.getWorkspaceRoot();
    ...
    try {
      const { readFilePreview } =
        await import("../adapters/workspace/fileSystem");
      ...
      try {
        ...
        for (const r of sorted) {
          ...
          if (lineIdx >= 0 && lineIdx < lines.length) {
            ...
            lines[lineIdx] =
              line.slice(0, r.column) +
              r.replaceText +
              line.slice(r.column + r.length);
          }
        }
  ...
  private async handleReplaceOne(
    ...
    const searchResult = this.lastSearchResults.find(
      (r) => r.path === filePath && r.line === line,
    );
    if (!searchResult) {
      vscode.window.showWarningMessage(
        `Could not find search result for ${filePath}:${line}`,
      );
      return;
    }

    const refreshed = await this.checkAndRefreshStaleFile(
      filePath,
      searchResult.mtime,
    );
    ...
    if (refreshed) {
      ...
      if (match) {
        ...
        vscode.window.showWarningMessage(
          `Match no longer exists in ${filePath}`,
        );
        this.rerunSearch();
        return;
      }
    ...
    if (result.success) {
      ...
      vscode.window.showWarningMessage(
        `Failed to replace in ${filePath}: ${result.error}`,
      );
    }
    ...
    this.postMessage({
      ...
      result: {
        ...
        replacedCount: result.success ? 1 : 0,
      },
    });
  }
  ...
  private async handleReplaceAll(replaceText: string): Promise<void> {
    ...
    const totalMatches = this.lastSearchResults.reduce(
